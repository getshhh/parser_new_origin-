...
class VKParserService:
    ...
    async def _parse_and_send(self) -> None:
        async with self.lock:
            ...
            if not channels:
                log.info("Нет настроенных каналов для VK.")
                return

            global_settings = self.db.get_vk_settings()
            group_ids = global_settings.get("group_ids", [])
                    ...
                    try:
                        ...
                        for channel in channels:
                            ...
                            for post in all_posts:
                                if self._filter_post(post, global_settings, channel):
    ...
    def _filter_post(self, post: Dict[str, Any], global_settings: dict, channel_settings: dict) -> bool:
        # Глобальные фильтры
        ...
        if global_keywords and not any(kw.lower() in title_and_desc for kw in global_keywords):
            return False
        ...
        if channel_keywords and not any(kw.lower() in title_and_desc for kw in channel_keywords):
            return False

        if channel_minus_keywords and any(kw.lower() in title_and_desc for kw in channel_minus_keywords):
            return False
