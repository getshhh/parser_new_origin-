...
async def settings_fl(callback: CallbackQuery, state: FSMContext = None):
    settings = db.get_fl_settings()
    parser_settings = db.get_parser_settings('fl')
...
⏰ Интервал сообщений: {interval_text}
    ...
    await callback.message.edit_text(text, reply_markup=settings_fl_kb())
...
async def process_fl_keywords(message: Message, state: FSMContext):
    ...
    settings = db.get_fl_settings()
    db.save_fl_settings(keywords, settings.get('min_price'), settings.get('max_price'))
...
# Выбор канала для FL
# -----------------------------
@router.callback_query(F.data == "set_fl_channel")
async def set_fl_channel(callback: CallbackQuery):
    ...
    buttons = [
        [InlineKeyboardButton(text=title, callback_data=f"fl_channel_select_{chat_id}")]
        for chat_id, title in channels
    ]
    buttons.append([InlineKeyboardButton(text="◀️ Назад", callback_data="settings_fl")])
    keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)

    await callback.message.edit_text("Выберите канал для отправки уведомлений FL:", reply_markup=keyboard)
    await callback.answer()

@router.callback_query(F.data.startswith("fl_channel_select_"))
async def process_fl_channel_selection(callback: CallbackQuery, state: FSMContext):
    channel_id = int(callback.data.split("_")[-1])
    db.set_parser_channel('fl', channel_id)

    await callback.answer("✅ Канал для FL успешно выбран!")
    await settings_fl(callback, state)
